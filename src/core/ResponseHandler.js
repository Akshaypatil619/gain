...
module.exports = class ResponseHandler {
	async response(req, res, result) {
		console.log("resultresultresultresultresult : ",result,result.status,req['activity_detail']);
		let client_request = {};
		...
		try {
			...
			if (req['is_success'] === true) {
				...
				if (activity_details) {
					...
					// if (activity_details.api_user === "tenant") {
							...
							let email_template_data_array = await knex('languages_for_email_templates')
								...
								try {
									for (let i = 0; i < email_template_data_array.length; i++) {
										...
										console.log("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF : ",email_body)
										send_mail.sending({

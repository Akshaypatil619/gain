...
module.exports = class TenantController {
    ...
    async add_tenant(req, res) {
        ...
        if (validation.passes() && !validation.fails()) {
            ...
            tenant_model.get_customer_id(form_data.customer_id).then(async custDetails => {
                ...
                let data = { templateCode: 'CREATEOWNERTENANT', customerDetails: tenantCustomer, templateDetails };
                ...
                if(ownerDeails.length>0){
                    ...
                    notif_instance.send_notifications({ templateCode: 'ADDOWNOWNER', customerDetails: ownerCustomer, templateDetails })
                                            .then(res => console.log(res)).catch(err => console.log(err));
    ...
    async list_tenant(req, res) {
        ...
        if (validation.passes() && !validation.fails()) {
            ...
            tenant_model.get_customer_id(form_data.customer_id).then(async custDetails => {
                ...
                let tenant_count = await tenant_model.get_tenant_list_count(form_data);
                let tenant_data = await tenant_model.get_tenant_list_data(form_data);
                if (!tenant_count.length) return res.json(tenant_response.failed("tenant_not_found"));

                const return_result = { total_records: tenant_count.length, tenant_list: tenant_data };
    ...
    async list_emirate(req, res) {
        let form_data = tenant_formatter.list_emirate(req);
        let rules = tenant_validator.list_emirate();
        let validation = new Validator(form_data, rules);
        if (validation.passes() && !validation.fails()) {
            let result = await tenant_model.list_emirate("list_emirate", form_data);
            if (result.length > 0) {
                return res.json(tenant_response.success("emirate_found", result));
            } else {
                return res.json(tenant_response.success("emirate_not_found"));
            }
        } else {
            return res.json(tenant_response.failed('form_fields_required', validation.errors.errors));
        }

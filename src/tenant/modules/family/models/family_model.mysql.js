...
module.exports = class FamilyModel {
    ...
    get_family_list(data) {
        let columns = {
            ...
            referral_type: "referrer_user_type.name",
        ...
        let obj = knex.select(columns)
            ...
            .join('master_user_type as referrer_user_type', 'referrer.user_type_id', 'referrer_user_type.id')
            .leftJoin('master_unit', 'referrer.id', knex.raw(`case when referrer_user_type.code = 'owner' then master_unit.customer_id else master_unit.tenant_customer_id end`))
            .leftJoin('master_building','master_building.id','master_unit.building_id')
            .whereRaw(`case when referrer_user_type.code = 'owner' then master_unit.is_default = 1 else 1 end`)
          
            .where("family.status",1);    
            if (data['owner_id']) {
                obj.andWhere("family.referrer_id", data['owner_id']);
            }
            if (data['name']) {
                obj.andWhereRaw("concat(CAST(AES_DECRYPT(family.first_name,'" + encription_key + "') AS CHAR(255)),'',CAST(AES_DECRYPT(family.last_name,'" + encription_key + "') AS CHAR(255))) like '%" + data['name'] + "%'")
            }
            if (data['email']) {
                obj.andWhereRaw("CAST(AES_DECRYPT(family.email,'" + encription_key + "') AS CHAR(255)) like '%" + data['email'] + "%'")
            }
            if (data['phone']) {
                obj.andWhereRaw("CAST(AES_DECRYPT(family.phone,'" + encription_key + "') AS CHAR(255)) like '%" + data['phone'] + "%'")
            }
            if (data['family_unique_id']) {
                obj.andWhereRaw("CAST(AES_DECRYPT(family.customer_unique_id,'" + encription_key + "') AS CHAR(255)) like '%" + data['family_unique_id'] + "%'")
            }
            if (data['building_id']) {
                obj.andWhere("master_unit.building_id",data['building_id'])
            }

            if (data['unit_no']) {
                obj.andWhere("master_unit.unit_no",data['unit_no'])
            }

            
            obj.groupBy('family.id');
            obj.orderBy("family.created_at", "DESC") //.limit(data['limit']).offset(data['offset'])
    ...
    async addFamily(form_data){
        ...
        if(customers[0].type=="Owner"){
            ...
            if(allowedData.length>0 && allowedData[0].property_type=="Commercial"){
                allowed=3;
                console.log("allowedData",allowedData,allowed)
    ...
    async get_total_familyList_count(data){

        let count_query = knex.select({total_record: knex.raw("COUNT(*)")})
        .from("customers as family")
        .join('customers as referrer', 'family.referrer_id', 'referrer.id')
        .join('master_user_type', 'referrer.user_type_id', 'master_user_type.id') 
        .where("family.status",1);    
        if (data['owner_id']) {
            count_query.andWhere("family.referrer_id", data['owner_id']);
        }
        if (data['name']) {
            count_query.andWhereRaw("concat(CAST(AES_DECRYPT(family.first_name,'" + encription_key + "') AS CHAR(255)),'',CAST(AES_DECRYPT(family.last_name,'" + encription_key + "') AS CHAR(255))) like '%" + data['name'] + "%'")
        }
        if (data['email']) {
            count_query.andWhereRaw("CAST(AES_DECRYPT(family.email,'" + encription_key + "') AS CHAR(255)) like '%" + data['email'] + "%'")
        }
        if (data['phone']) {
            count_query.andWhereRaw("CAST(AES_DECRYPT(family.phone,'" + encription_key + "') AS CHAR(255)) like '%" + data['phone'] + "%'")
        }
        if (data['family_unique_id']) {
            count_query.andWhereRaw("CAST(AES_DECRYPT(family.customer_unique_id,'" + encription_key + "') AS CHAR(255)) like '%" + data['family_unique_id'] + "%'")
        }
        if (data['building_id']) {
            count_query.andWhere("master_unit.building_id",data['building_id'])
        }

        if (data['unit_no']) {
            count_query.andWhere("master_unit.unit_no",data['unit_no'])
        }
       return count_query;//count_query.groupBy("family.id").orderBy("family.created_at", "DESC");
    }

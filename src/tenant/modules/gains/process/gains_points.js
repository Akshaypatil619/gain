...
let errorResponse = require('../../../../tenant/modules/gains/responses/gains.response');
...
let handBackArray = [], gainsProcessFiles = [];
...
module.exports.startProcess = async function (callback) {
    ...
    fs.readdir(pointsDataDir, async (err, filesList) => {
        if(err) {
            ...
            console.log('gains accrual points flat file processing started.');
            callback();
            ...
            for(let customerValue of customerArray)
                customerObj['customer_'+customerValue.gains_customer_id] = customerValue;
            for(let transactionValue of transactionArray)
                transactionIdArray.push(transactionValue.gains_transaction_id);
                ...
                for(let record of workSheetData) {
                    ...
                    if(validation.passes() && !validation.fails()) {
                        ...
                        global.rule_engine_instance.process_activity({
                            ...
                            gains_source_id: activityObj[record.TRAN_POINTS_SOURCE_ID]['id'],
                            gains_tran_date: record.TRAN_DATE,
                            gains_tran_id: record.TRAN_ID
                        }, async ruleData => {
                            let obj = {
                                customer_id: ruleData.customer_id,
                                transaction_id: ruleData.transaction_id,
                                transaction_type: ruleData.transaction_type,
                                transaction_date: ruleData.gains_tran_date,
                                ...
                                gains_source_id: ruleData.gains_source_id,
                                rule_id: ruleData.rule_id,
                                gains_transaction_id: ruleData.gains_tran_id,
...
function getAllCustomers() {
    return knex('customers').select({
        gains_customer_id: 'gains_customer_id',
        id: 'id'
        }).where('status',1);
}
...
function getAllTransactionIds() {
    return knex('customer_transaction')
        .select(knex.raw('DISTINCT gains_transaction_id'));
}

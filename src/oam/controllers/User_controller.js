...
module.exports = class User_controller {
    ...
    async get_unitlist(req, res) {
        let return_response = {};

        const formData = formatter.get_unitlist(req);
        const rules = validator.get_unitlist();

        const validation = new Validator(formData, rules);
        if (validation.passes() && !validation.fails()) {

            return_response = await model.get_unitlist(formData);
    ...
    async get_unitReportlist(req, res) {
        let return_response = {};
        ...
        if (validation.passes() && !validation.fails()) {
            ...
            return_response = await model.get_unitReportlist(formData);
    ...
    async get_transaction_unitlist(req, res) {
        let return_response = {};
        ...
        if (validation.passes() && !validation.fails()) {
            ...
            return_response = await model.get_transaction_unitlist(formData);

...
let Queries = require("../queries/mysql/oam_login");
...
module.exports = class Oam_login_model {
               ...
               let passwordVerifivcation = await queries.check_login("check_credentials",form_data)
                    if(passwordVerifivcation.length>0){
                        if (passwordVerifivcation.length === 0) {
                            ...
                            result[0]['token'] = await common_functions.create_token({ email: form_data['email'],oam_id :result[0]['oam_id'], phone: form_data['phone']});
